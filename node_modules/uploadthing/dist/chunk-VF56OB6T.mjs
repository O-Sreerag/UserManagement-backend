import {
  getFullApiUrl
} from "./chunk-4QCB7YXI.mjs";
import {
  __require
} from "./chunk-JW5CQVQS.mjs";
...
import { isDevelopment, process as process2 } from "std-env";
import {
...
var buildRequestHandler = (opts, adapter) => {
  return async (input) => {
    var _a, _b;
    const isDev = ((_a = opts.config) == null ? void 0 : _a.isDev) ?? isDevelopment;
    const fetch = ((_b = opts.config) == null ? void 0 : _b.fetch) ?? globalThis.fetch;
    if (isDev) {
      logger.info("UploadThing dev server is now running!");
    }
    const { router, config } = opts;
    const preferredOrEnvSecret = (config == null ? void 0 : config.uploadthingSecret) ?? process2.env.UPLOADTHING_SECRET;
    ...
    switch (actionType) {
      case "upload": {
        ...
        try {
          ...
          if (error instanceof UploadThingError2)
            return error;
          return new UploadThingError2({
        ...
        if (!Array.isArray(files) || !files.every(
          (f) => isObject2(f) && typeof f.name === "string" && typeof f.size === "number"
        )) {
...
function resolveCallbackUrl(opts) {
  ...
  } else if (process2.env.UPLOADTHING_URL) {
    callbackUrl = getFullApiUrl(process2.env.UPLOADTHING_URL);
  }
...
function incompatibleNodeGuard() {
  ...
  let major;
  let minor;
  const maybeNodeVersion = (_b = (_a = process3.versions) == null ? void 0 : _a.node) == null ? void 0 : _b.split(".");
  if (maybeNodeVersion) {
    [major, minor] = maybeNodeVersion.map((v) => parseInt(v, 10));
  }
  const maybeNodePath = (_c = process3.env) == null ? void 0 : _c.NODE;
  ...
  (_f = (_e = process3).exit) == null ? void 0 : _f.call(_e, 1);
}
