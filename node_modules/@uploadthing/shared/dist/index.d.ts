...
type ExpandedRouteConfig = Partial<{
 ...
 * AN INFINITE LOOP
 */
declare const withExponentialBackoff: <T>(doTheThing: () => Promise<T | undefined>, MAXIMUM_BACKOFF_MS?: number, MAX_RETRIES?: number) => Promise<T | null>;
declare function pollForFileData(opts: {
    url: string;
    apiKey: string | null;
    sdkVersion: string;
    fetch: FetchEsque;
}, callback?: (json: any) => Promise<any>): Promise<symbol | null>;
declare const FILESIZE_UNITS: readonly ["B", "KB", "MB", "GB"];
type FileSizeUnit = (typeof FILESIZE_UNITS)[number];
declare const fileSizeToBytes: (input: string) => number | Error;
...
declare class UploadThingError<TShape extends Json = {
    ...
    readonly data?: TShape;
    constructor(initOpts: UploadThingErrorOptions<TShape> | string);
    static fromResponse(response: ResponseEsque): Promise<UploadThingError<{
        message: string;
    }> | UploadThingError<string | number | boolean | JsonObject | JsonArray | null>>;
    static toObject(error: UploadThingError): {
        code: "BAD_REQUEST" | "NOT_FOUND" | "FORBIDDEN" | "INTERNAL_SERVER_ERROR" | "INTERNAL_CLIENT_ERROR" | "TOO_LARGE" | "TOO_SMALL" | "TOO_MANY_FILES" | "KEY_TOO_LONG" | "URL_GENERATION_FAILED" | "UPLOAD_FAILED" | "MISSING_ENV" | "FILE_LIMIT_EXCEEDED";
        message: string;
        data: {
            message: string;
        } | undefined;
...
declare function getStatusCodeFromError(error: UploadThingError<any>): 500 | 400 | 404 | 403 | 413;

export { ACL, ALLOWED_FILE_TYPES, AllowedFileType, ContentDisposition, EndpointMetadata, ExpandedRouteConfig, FILESIZE_UNITS, FetchEsque, FileData, FileRouterInputConfig, FileRouterInputKey, FileSize, FileSizeUnit, Json, JsonArray, JsonObject, JsonValue, MaybeUrl, Overwrite, RequestInitEsque, ResponseEsque, SizeUnit, UploadThingError, UploadedFile, WithRequired, fileSizeToBytes, fillInputRouteConfig, generateUploadThingURL, getDefaultSizeForType, getStatusCodeFromError, getTypeFromFileName, isObject, isRouteArray, objectKeys, pollForFileData, safeParseJSON, withExponentialBackoff };
